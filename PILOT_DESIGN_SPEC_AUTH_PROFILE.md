...
4. **Profile tab row** (horizontal, scrollable, gap 22): Profile (active: Inter Bold 15 ink + 54px ink underline bar), Shows, Lists, Watchlist (inactive: Inter Medium 15 `muted`, count chips Inter Bold 10 `muted` on `hairline`). Full-width `hairline` under. _IA revised from the original Profile/Activity/Watched/Diary/Reviews: **Shows** is a grid of watched shows (review-badged), **Watchlist** and **Lists** are their own tabs, and Activity/Diary/Following/Followers became scroll-down sub-pages off the Profile tab (Diary = the chronological, date-grouped view). Comments dropped — no system._
5. **"Your Top 4"** — Archivo Black 20 ink (left) + "Edit" Inter SemiBold 14 `purple` (right). Below: a row of **4 dashed empty slots** sized to fit the content width with **no horizontal scroll** (≈80×108, keeping the ~98:132 poster ratio), radius 8, fill `field`, 1.5px `dashStroke` dashed border (dash 5 / gap 4), centered slot number Inter Bold 18 `dashStroke`. (Filled state: replace with show poster.) _Changed from Top 5 → Top 4 so all favorites are visible at once without scrolling — matches Letterboxd given the 2:3 poster aspect._
